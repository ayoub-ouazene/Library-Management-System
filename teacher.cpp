...
Teacher::Teacher(const QString& a,const QString& b,const QString& c,const QString& d,const QString& e,const QString& f,const QString& g,int x,int y,long long z)
    :User(a,b,c,d,e,f,g,x,y,z){}

int Teacher::getTotalNumberOfTeachers()
{
    QSqlQuery query;

    if (query.exec("SELECT COUNT(*) FROM users WHERE USER_TYPE = 'teacher'")) {
        if (query.next()) {
...
QList<Teacher> Teacher::getAllTeachers() {
    QList<Teacher> userList;
    ...
    if (query.exec("SELECT FirstName, LastName, ID_CARD, RFID, UNIVERSITY, EMAIL, "
        ...
        while (query.next()) {
            ...
            Teacher u(firstName, lastName, email, password_db, dateOfBirth, university,
                      "", 0, rfid, idCard);
            userList.append(u);
        }
    } else {
